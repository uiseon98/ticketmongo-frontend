...
import { X, Calendar, Clock, MapPin, Users, Image, AlertCircle, CheckCircle } from 'lucide-react';
...
const ConcertForm = ({
  ...
  const validateForm = () => {
    ...
    if (!formData.bookingStartDate) {
      ...
      if (bookingStart <= now) {
        newErrors.bookingStartDate = '예매 시작일시는 현재 시간보다 이후여야 합니다';
      }
    }
    ...
    if (!formData.bookingEndDate) {
      ...
      if (bookingEnd <= now) {
        newErrors.bookingEndDate = '예매 종료일시는 현재 시간보다 이후여야 합니다';
      }
    }
    ...
    if (formData.bookingStartDate && formData.bookingEndDate) {
      ...
      if (bookingEnd <= bookingStart) {
        newErrors.bookingEndDate = '예매 종료일시는 예매 시작일시보다 늦어야 합니다';
      }
    }
    ...
    if (formData.bookingEndDate && formData.concertDate && formData.startTime) {
      const bookingEnd = new Date(formData.bookingEndDate);
      const concertStart = new Date(`${formData.concertDate}T${formData.startTime}`);
    ...
    if (isNaN(maxTickets) || maxTickets < 1) {
      newErrors.maxTicketsPerUser = '사용자당 최대 티켓 수는 1개 이상이어야 합니다';
    } else if (maxTickets > 10) {
      newErrors.maxTicketsPerUser = '사용자당 최대 티켓 수는 10개 이하여야 합니다';
    }
    ...
    if (formData.posterImageUrl) {
      ...
      if (!urlPattern.test(formData.posterImageUrl)) {
        newErrors.posterImageUrl = '포스터 이미지 URL은 올바른 이미지 URL 형식이어야 합니다';
      }
    }
  ...
  const updateConcert = async () => {
    ...
    if (formData.venueName?.trim()) updateData.venueName = formData.venueName.trim();
    if (formData.venueAddress !== undefined) updateData.venueAddress = formData.venueAddress?.trim() || null;
    ...
    if (formData.totalSeats) updateData.totalSeats = parseInt(formData.totalSeats);
    if (formData.bookingStartDate) updateData.bookingStartDate = formData.bookingStartDate;
    if (formData.bookingEndDate) updateData.bookingEndDate = formData.bookingEndDate;
    if (formData.minAge !== undefined) updateData.minAge = parseInt(formData.minAge);
    if (formData.maxTicketsPerUser !== undefined) updateData.maxTicketsPerUser = parseInt(formData.maxTicketsPerUser);
    if (formData.status) updateData.status = formData.status;
    if (formData.posterImageUrl !== undefined) updateData.posterImageUrl = formData.posterImageUrl?.trim() || null;

    const response = await fetch(`/api/seller/concerts/${concert.concertId}?${params}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        setSubmitSuccess(result.message || (isEditMode ? '콘서트가 수정되었습니다.' : '콘서트가 생성되었습니다.'));
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              />
              {errors.venueAddress && (
                <p className="mt-1 text-sm text-red-600">{errors.venueAddress}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.concertDate && (
                <p className="mt-1 text-sm text-red-600">{errors.concertDate}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.bookingStartDate && (
                <p className="mt-1 text-sm text-red-600">{errors.bookingStartDate}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.bookingEndDate && (
                <p className="mt-1 text-sm text-red-600">{errors.bookingEndDate}</p>
              )}
            </div>
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">0세는 연령 제한 없음을 의미합니다</p>
            </div>
            ...
            <div>
              ...
              {errors.maxTicketsPerUser && (
                <p className="mt-1 text-sm text-red-600">{errors.maxTicketsPerUser}</p>
              )}
            </div>
