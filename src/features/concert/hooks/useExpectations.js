...
export const useExpectations = (concertId) => {
   ...
   */
  const fetchExpectations = useCallback(async (params = {}) => {
    try {
      ...
      if (response && response.data) {
        ...
        console.info(`기대평 목록 로드 완료: ${response.data.content?.length || 0}개 (콘서트 ID: ${concertId})`);
      } else {
      ...
      setError(err.message || '기대평 목록을 불러오는 중 오류가 발생했습니다.');
  ...
  }, [concertId, currentPage, pageSize]); // 이 값들이 변경되면 함수 재생성
   ...
   */
  const createExpectation = useCallback(async (expectationData) => {
    try {
      // concertId 유효성 검증
      if (!concertId || concertId < 1) {
        throw new Error('유효한 콘서트 ID가 필요합니다.');
      ...
      const response = await expectationService.createExpectation(concertId, expectationData);
      ...
      // 성공 로그
      console.info(`기대평 작성 완료: 기대점수 ${expectationData.expectationRating}점 (콘서트 ID: ${concertId})`);
  ...
  }, [concertId, fetchExpectations]); // concertId와 fetchExpectations가 변경되면 함수 재생성
   ...
   */
  const updateExpectation = useCallback(async (expectationId, expectationData) => {
    try {
      // ID 파라미터 유효성 검증
      if (!concertId || concertId < 1) {
        throw new Error('유효한 콘서트 ID가 필요합니다.');
      ...
      const response = await expectationService.updateExpectation(concertId, expectationId, expectationData);
      ...
      // 성공 로그
      console.info(`기대평 수정 완료: ID ${expectationId} (콘서트 ID: ${concertId})`);
   ...
   */
  const deleteExpectation = useCallback(async (expectationId) => {
    try {
      // ID 파라미터 유효성 검증
      if (!concertId || concertId < 1) {
        throw new Error('유효한 콘서트 ID가 필요합니다.');
      ...
      // 성공 로그
      console.info(`기대평 삭제 완료: ID ${expectationId} (콘서트 ID: ${concertId})`);
  ...
  }, [concertId, fetchExpectations, expectations.length, currentPage]); // 의존하는 상태들
  ...
  const goToPage = useCallback(async (newPage) => {
    ...
    if (newPage < 0 || newPage >= totalPages) {
      console.warn(`유효하지 않은 페이지 번호: ${newPage} (범위: 0-${totalPages - 1})`);
      return;
    }
  ...
  }, [fetchExpectations, totalPages, currentPage]); // 이 값들이 변경되면 함수 재생성
  ...
  }, [fetchExpectations, pageSize]); // 이 값들이 변경되면 함수 재생성
  ...
  const filterByRating = useCallback((rating) => {
    ...
    // 특정 기대점수로 필터링
    return expectations.filter(expectation => expectation.expectationRating === rating);
  }, [expectations]); // expectations가 변경되면 함수 재생성
