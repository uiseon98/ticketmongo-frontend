...
export const useReviews = (concertId) => {
  ...
  const fetchReviews = useCallback(async (params = {}) => {
    try {
      ...
      if (response && response.data) {
        ...
        console.info(`리뷰 목록 로드 완료: ${response.data.content?.length || 0}개 (콘서트 ID: ${concertId})`);
      } else {
  ...
  const createReview = useCallback(async (reviewData) => {
    try {
      ...
      const response = await reviewService.createReview(concertId, reviewData);
      ...
      // 성공 로그
      console.info(`리뷰 작성 완료: "${reviewData.title}" (콘서트 ID: ${concertId})`);
  ...
  }, [concertId, fetchReviews]); // concertId와 fetchReviews가 변경되면 함수 재생성
  ...
  const updateReview = useCallback(async (reviewId, reviewData) => {
    try {
      ...
      const response = await reviewService.updateReview(concertId, reviewId, reviewData);
      ...
      // 성공 로그
      console.info(`리뷰 수정 완료: ID ${reviewId} (콘서트 ID: ${concertId})`);
  ...
  const deleteReview = useCallback(async (reviewId) => {
    try {
      ...
      // 성공 로그
      console.info(`리뷰 삭제 완료: ID ${reviewId} (콘서트 ID: ${concertId})`);
  ...
  }, [concertId, fetchReviews, reviews.length, currentPage]); // 의존하는 상태들
  ...
  }, [fetchReviews, totalPages, currentPage]); // 이 값들이 변경되면 함수 재생성
   ...
   */
  const changeSorting = useCallback(async (newSortBy, newSortDir = 'desc') => {
    // 정렬 기준 유효성 검증
  ...
  }, [fetchReviews, sortBy, sortDir]); // 이 값들이 변경되면 함수 재생성
  ...
  }, [fetchReviews, pageSize]); // 이 값들이 변경되면 함수 재생성
