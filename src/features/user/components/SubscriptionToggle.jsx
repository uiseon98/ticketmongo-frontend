    ...
    const syncStates = async () => {
      ...
      try {
        ...
        if (OneSignal && OneSignal.User?.PushSubscription) {
          oneSignalSubscribed = await OneSignal.User.PushSubscription.optedIn;
  ...
  const handleToggle = async () => {
    ...
    try {
      if (!isSubscribed) {
        ...
        // 해제: DB → OneSignal
        const playerId = OneSignal.User.PushSubscription.id || localStorage.getItem(LOCAL_PLAYER_ID_KEY);
        if (playerId) {
  ...
  return (
    <div>
      ...
      <label className="flex items-center space-x-3 mt-4 cursor-pointer">
        ...
        />
        <span className="text-gray-200 font-medium">서비스 알림 수신 동의 </span>
      </label>
      <p className="text-sm text-gray-400 mt-10">
        브라우저 알림 권한: <strong className={permission === 'granted' ? 'text-green-600' : 'text-red-600'}>{permission}</strong>
        <br />
