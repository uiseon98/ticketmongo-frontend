...
export function BookingStatus({ status, timeUntilConcert }) {
    ...
    return (
        ...
        >
            ...
            <span className={`font-bold ${config.textColor} text-lg`}>{config.text}</span>
            {status === 'CONFIRMED' && <span className="text-sm text-gray-600 mt-1">공연까지 {timeUntilConcert}</span>}
        </div>
    );
}
